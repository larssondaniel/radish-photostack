...
static CGFloat const PhotoRotationOffsetDefault = 2.0f;
...
-(void)rotatePhoto:(UIView *)photo degrees:(NSInteger)degrees animated:(BOOL)animated {
    ...
    float scaleFactor = ((self.frame.size.width-50)/photo.frame.size.width);
    ...
    if(animated) {
        [UIView animateWithDuration:0.2
                         animations:^{
                             photo.transform = rotateTransform;
                         }
         completion:^(BOOL finished) {
             CGAffineTransform transform = CGAffineTransformMakeTranslation(0, -50);
             CGAffineTransform transform2 = CGAffineTransformMakeScale(scaleFactor, scaleFactor);
             CGAffineTransform combinedTranform = CGAffineTransformConcat(transform, transform2);
             
             [UIView animateWithDuration:0.3
                              animations:^{
                                  photo.transform = combinedTranform;
                              } completion:^(BOOL finished) {
...
-(void)makeCrooked:(UIView *)photo animated:(BOOL)animated {
    ...
    if (photo.frame.size.width > photo.frame.size.height)
        degrees += 90;
...
-(void)reloadData {
    ...
    if(numberOfPhotos > 0) {
        ...
        for (NSUInteger index = 0; index < numberOfPhotos; index++) {
            ...
            //NSLog(@"Size is %f times %f", imageSize.width, imageSize.height);
            if (imageSize.height > 350) {
                photoImageView     = [[UIImageView alloc] initWithFrame:(CGRect){CGPointZero, CGSizeMake(200, imageSize.height/(imageSize.width/200))}];
            } else {
                photoImageView     = [[UIImageView alloc] initWithFrame:(CGRect){CGPointZero, imageSize}];
            }
            ...
            if ([photoViewsMutable objectAtIndex:0] == view)
                [self makeCrooked:view animated:YES];
        }
